package pl.chmielewskiadam;

public class CRC {

    private int crcRegister = 0xFFFF;

    public int computing_Crc(byte[] buf, int len) {

        for (int pos = 0; pos < len; pos++) {
            crcRegister ^= (int) buf[pos];

            for (int i = 8; i != 0; i--) {    // Loop over each bit
                if ((crcRegister & 0x0001) != 0) {      // If the LSB is set
                    crcRegister >>= 1;                    // Shift right and XOR 0xA001
                    crcRegister ^= 0xA001;
                } else                            // Else LSB is not set
                    crcRegister >>= 1;                    // Just shift right
            }
               }
            int crc8 = crcRegister >> 8;
        int swapped = ((crcRegister >> 8) & 0xff) | ((crcRegister & 0xff) << 8);
            // Note, this number has low and high bytes swapped, so use it accordingly (or swap bytes)
            return swapped;

        }

        public int computingCRCtable(byte[] buf, int len){
            int[] aCRCHi =
                    {
                            
                    };
            int aCRCLo[] =
                    {
                            
                    };

            int HiByte = 0xFF;
            int LoByte = 0xFF;
            int Index;
            for(int i = 0 ; len>i ; i++)
            {

                Index = HiByte ^ buf[i];
                HiByte = LoByte ^ aCRCHi[Index];
                LoByte = aCRCLo[Index];

            };
            return (HiByte << 8 | LoByte);
        }
    public static int crc16(final byte[] buffer) {
        /* Note the change here */
        int crc = 0x1D0F;
        for (int j = 0; j < buffer.length ; j++) {
            crc = ((crc  >>> 8) | (crc  << 8) )& 0xffff;
            crc ^= (buffer[j] & 0xff);//byte to int, trunc sign
            crc ^= ((crc & 0xff) >> 4);
            crc ^= (crc << 12) & 0xffff;
            crc ^= ((crc & 0xFF) << 5) & 0xffff;
        }
        crc &= 0xffff;
        return crc;
    }
        }

